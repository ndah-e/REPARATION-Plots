...
run_randomforest <- function(train,pos,neg){
	...
	rf.output <- randomForest(class~., data=train, ntree=4001, mtry=3, classwt = c(P=pos, N=neg))
	return(rf.output)
}
...
#true <- sample(c("N","P"),15,replace=TRUE)
#pred <- sample(c("N","P"),15,replace=TRUE)

#t <- table(pred,true)
#sen <- t[2,2]/(t[1,2] + t[2,2])
#sen
#pres  <- t[2,2]/(t[2,1] + t[2,2])
#pres
...
classwt <- seq(0.1,0.9,0.1) 
for (j in 1:length(classwt)) {

	salt_mono_cv_error <- vector()
	ecoli_cv_error <- vector()

	cat("Processing classwt ",classwt[j],"\n", sep=" ")
	...
	for(i in 1:nfolds) {
		...
		ecoli.pos <- classwt[j]
		ecoli.neg <- 1 - classwt[j]

		rf.ecoli <- run_randomforest(trainset.ecoli,ecoli.pos,ecoli.neg)
		...
		salt.pos <- classwt[j]
		salt.neg <- 1 - classwt[j]
		rf.salt_mono <- run_randomforest(trainset.salt_mono,salt.pos,salt.neg)
...
df <- data.frame(classwt=classwt,ecoli=ecoli,salt_mono=salt_mono)
write.table(df, file="classwt.txt", sep = "\t",col.names = TRUE,row.names = F, quote = FALSE)

min.ecoli <- paste(df[which(df$ecoli == min(df$ecoli)),]$classwt, collapse =" ")
min.salt <- paste(df[which(df$salt == min(df$salt)),]$classwt, collapse =" ")

cat("Ecoli ", min.ecoli, "\n")
cat("Salt ", min.salt, "\n")

pdf(file="classwt_tuning.pdf")
ymax <- max(df[,-1])
ymin <- min(df[,-1])
plot(df[,1],df[,2], xlab="classwt", ylab="Precision", col=2, pch=16, ylim=c(ymin,ymax))
