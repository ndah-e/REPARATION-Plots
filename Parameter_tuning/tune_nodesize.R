...
run_randomforest <- function(train,pos,neg,nodesize){
	set.seed(12345)	# set seed
	rf.output <- randomForest(class~., data=train,nodesize=nodesize,classwt=c(P=pos, N=neg))
	return(rf.output)
}
...
#nodesize <- seq(11,30,3) 
nodesize <- c(c(1:10),seq(11,35,3))
for (j in 1:length(nodesize)) {

	salt_mono_cv_error <- vector()
	ecoli_cv_error <- vector()

	cat("Processing nodesize ",nodesize[j],"\n", sep=" ")
	...
	for(i in 1:nfolds) {
		...
		pos.prob.ecoli <- round(dim(trainset.ecoli.p)[1]/dim(trainset.ecoli.n)[1],digit=2)
		neg.prob.ecoli <- 1 - pos.prob.ecoli

		rf.ecoli <- run_randomforest(trainset.ecoli,pos.prob.ecoli,neg.prob.ecoli,nodesize[j])
		...
		pos.prob.salt_mono <- round(dim(trainset.salt_mono.p)[1]/dim(trainset.salt_mono.n)[1],digit=2)
		neg.prob.salt_mono <- 1 - pos.prob.salt_mono

		rf.salt_mono <- run_randomforest(trainset.salt_mono,pos.prob.salt_mono,neg.prob.salt_mono,nodesize[j])
...
df <- data.frame(nodesize=nodesize,ecoli=ecoli,salt_mono=salt_mono)
write.table(df, file="nodesize.txt", sep = "\t",col.names = TRUE,row.names = F, quote = FALSE)

min.ecoli <- paste(df[which(df$ecoli == min(df$ecoli)),]$nodesize, collapse =" ")
min.salt <- paste(df[which(df$salt == min(df$salt)),]$nodesize, collapse =" ")

cat("Ecoli ", min.ecoli, "\n")
cat("Salt ", min.salt, "\n")

pdf(file="nodesize_tuning.pdf")
ymax <- max(df[,-1])
ymin <- min(df[,-1])
plot(df[,1],df[,2], xlab="nodesize", ylab="CV Error rate", col=2, pch=16, ylim=c(ymin,ymax))
