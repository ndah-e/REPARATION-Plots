...
run_randomforest <- function(train,pos,neg,mtry){
	...
	rf.output <- randomForest(class~., data=train, mtry=mtry,classwt = c(P=pos, N=neg))
	return(rf.output)
}
...
for (j in 1:length(mtry)) {
	...
	for(i in 1:nfolds) {
		...
		rf.ecoli <- run_randomforest(trainset.ecoli,pos.prob.ecoli,neg.prob.ecoli,mtry[j])
		...
		rf.salt_mono <- run_randomforest(trainset.salt_mono,pos.prob.salt_mono,neg.prob.salt_mono,mtry[j])
...
df <- data.frame(mtry=mtry,ecoli=ecoli,salt_mono=salt_mono)
write.table(df, file="mtry.txt", sep = "\t",col.names = TRUE,row.names = F, quote = FALSE)

min.ecoli <- paste(df[which(df$ecoli == min(df$ecoli)),]$mtry, collapse =" ")
min.salt <- paste(df[which(df$salt == min(df$salt)),]$mtry, collapse =" ")

cat("Ecoli ", min.ecoli, "\n")
cat("Salt ", min.salt, "\n")

pdf(file="mtry_tuning.pdf")
ymax <- max(df[,-1])
ymin <- min(df[,-1])
plot(df[,1],df[,2], xlab="mtry", ylab="precision", col=2, pch=16, ylim=c(ymin,ymax))
